...
class TestSearchHistory:
    ...
    async def test_empty_history(self, replica_session):
        user_id = randint(1, 1000)
        assert await get_search_history(replica_session, user_id, 1) == []

    async def test_first_page(self, replica_session, user: User):
    ...
    async def test_not_full_page(self, replica_session, user: User):
